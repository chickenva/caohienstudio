...
// 2. Gửi mail hợp đồng
// Gửi email hợp đồng, link xác nhận và link PDF cho khách hàng.
/**
 * Hàm gửi email chứa hợp đồng và link xác nhận cho khách hàng.
 * Xử lý: Đính kèm URL hợp đồng, file PDF và mã QR để khách xem.
 * @param {Object} booking - Thông tin đơn đặt lịch
 * @param {Object} customer - Thông tin khách hàng
  ...
  try {
    ...
    const content = `
      ...
      <div style="background-color: #fffbe6; border-left: 4px solid #faad14; padding: 15px; margin-top: 25px; border-radius: 4px;">
        <p style="margin: 0; font-size: 14px; color: #d48806;">Lưu ý: Hợp đồng chỉ có hiệu lực và lịch chụp chỉ được giữ chính thức sau khi bạn xác nhận và hoàn tất thanh toán tiền cọc.</p>
      </div>
    `;
    ...
    const mailOptions = {
      from: `"Cao Hien Studio" <${process.env.EMAIL_USER}>`,
      to: customer.email,
      subject: `[Cao Hien Studio] Hợp đồng dịch vụ nhiếp ảnh (Mã đơn: #${booking._id.toString().slice(-8).toUpperCase()})`,
